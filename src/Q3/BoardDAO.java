...
public class BoardDAO {
	...
	//select method
	//부여된 index의 객체의 title과 content를 출력하는 method
	...
	public void select () {//override : index 지정 안할 경우 전체 출력 method
	...
	public void update(String title, String content) {
		for (int i = 0; i < this.getBoardList().size(); i++) {
			if (title.equals(this.getBoardList().get(i).getTitle())) {
				...
				System.out.println("------------------");
				System.out.printf("'%s'의 내용이 수정 되었습니다\n",title);
				return;
			} 
		}
	...
	//delete method
	//title을 받아 dto 객체 중 같은 title을 가진 객체를 찾아 삭제
	public void delete(String title) {
		for (int j = 0; j < this.getBoardList().size(); j++) {
			if (title.equals(this.getBoardList().get(j).getTitle())) {
				...
				System.out.println("------------------");
				System.out.printf("'%s'이 삭제 되었습니다.\n",title);
				return;
			}
		}
		System.out.println("해당 제목의 글을 찾을 수 없습니다");
	}
	//현 DAO 코드는 중복되는 제목을 가진 글에 대해서 구현 못하는 한계
	//따라서 객체마다 고유값(ID Primary key)을 부여함이 바람직 할것.
